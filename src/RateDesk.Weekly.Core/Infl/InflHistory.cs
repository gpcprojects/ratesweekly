...
namespace RateDesk.Weekly.Core.Infl
{
    ...
    public static class InflHistory
    {
        ...
        // ------------------------------------------------------------------ xlsm ingest ----
        public static IngestResult Ingest(string workbookPath, HistoryStore store, Action<string>? log = null)
        {
            ...
            try { return IngestCopy(tmp, store, log); }
            ...
            foreach (var fam in Families)
            {
                ...
                foreach (var row in ws.RowsUsed().Skip(1))
                {
                    ...
                    if (baseV is { } b)
                    {
                        ...
                        else if (own is null) noCheck++;   // print not in store — kept on label
                ...
                foreach (var (fix, pts) in byFix)
                    ingested += store.UpsertFixings(fam.Key, fix, pts, "xls");
